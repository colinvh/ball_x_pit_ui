...
<div class="relative inline-block">
  ...
  {#if showTooltip && (text || tooltip)}
    <div
      role="tooltip"
      class="absolute z-50 px-3 py-2 text-sm text-white bg-gray-900 rounded shadow-lg bottom-full left-1/2 transform -translate-x-1/2 mb-1 w-64 break-words text-center"
    >
      {#if tooltip}
        {@render tooltip()}
      {:else}
